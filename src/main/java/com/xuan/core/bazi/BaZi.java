...
//====================================================================================================================================================
    ...
    private void core(BaZiSetting setting) {
        ...
        diZhiLiuYi(); // 计算地支留意
        qiYun(setting); // 计算起运
        jieQi(); // 计算上一节及日期、下一节及日期、出生节气 TODO：当天数小于1天时会出现显示bug，待修复。
...
//        System.out.println("身体强弱：" + bodyIntensity);
    ...
    private void guZhong() {
        ...
        if ("男".equals(getSex())) {
            ...
            Map<Integer, String> yunChengWan = BaZiMap.MAN_GU_ZHONG_YUAN_WEN; // 骨重批注（男命）
            ...
            Map<Integer, String> yunChengWan = BaZiMap.WOMAN_GU_ZHONG_YUAN_WEN; // 骨重批注（女命）
            this.guZhong = guZhongCharacters(guZhong); // 骨重转为文字
            this.guZhongPiZhu = yunChengWan.get(guZhong);
        }
