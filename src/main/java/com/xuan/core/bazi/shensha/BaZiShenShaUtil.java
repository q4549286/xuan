...
//****************************************************************************************************************************************************
    ...
    public BaZiShenShaUtil(List<String> baZi, String yearGanZhiNaYinWuXing, String season, String sex) {
        ...
        if (baZi.size() == 4) {
            ...
            this.yearGanZhi = (null != baZi.get(0) && baZi.get(0).length() == 2) ? baZi.get(0) : "--"; // 年干支
            this.yearGan = getYearGanZhi().substring(0, 1); // 年干
            this.yearZhi = getYearGanZhi().substring(1, 2); // 年支
            this.monthGanZhi = (null != baZi.get(1) && baZi.get(1).length() == 2) ? baZi.get(1) : "--"; // 月干支
            this.monthGan = getMonthGanZhi().substring(0, 1); // 月干
            this.monthZhi = getMonthGanZhi().substring(1, 2); // 月支
            this.dayGanZhi = (null != baZi.get(2) && baZi.get(2).length() == 2) ? baZi.get(2) : "--"; // 日干支
            this.dayGan = getDayGanZhi().substring(0, 1); // 日干
            this.dayZhi = getDayGanZhi().substring(1, 2); // 日支
            this.hourGanZhi = (null != baZi.get(3) && baZi.get(3).length() == 2) ? baZi.get(3) : "--"; // 时干支
