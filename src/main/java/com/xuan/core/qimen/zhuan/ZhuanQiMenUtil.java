    ...
    public static Map<String, Map<Integer, List<List<String>>>> tianPanQiYiLuoGongLink(List<String> tianPanQiYiToTq, List<String> tianPanQiYiExTq) {
        ...
        for (int i = 0; i < 9; i++) {
            if (i != 4) {
                ...
                if (null == statusToTq) {
                    mapToTq.put(i, Arrays.asList(Arrays.asList("", ""), Arrays.asList("", "")));
                } else {
                    mapToTq.put(i, oneList2ToTq);
                }
                mapExTq.put(i, oneList2ExTq);
            } else {
                mapToTq.put(i, Arrays.asList(Arrays.asList("", ""), Arrays.asList("", ""))); // 中五宫
                mapExTq.put(i, Arrays.asList(Arrays.asList("", ""), Arrays.asList("", ""))); // 中五宫
            }
        }
